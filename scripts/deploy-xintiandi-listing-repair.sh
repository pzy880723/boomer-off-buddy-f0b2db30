...
old="$base/releases/xintiandi-listing-repair-20260924"
release="$base/releases/xintiandi-listing-repair-v2-20260924"
candidate=boomer-xintiandi-repair-candidate
patch=/tmp/boomer-xintiandi-repair-patch

check() {
  local port="$1" ready=0
  for attempt in $(seq 1 30); do
    if curl -fsS --max-time 5 "http://127.0.0.1:$port/api/public/handheld/openapi.json" -o /tmp/boomer-xintiandi-openapi.json; then ready=1; break; fi
    sleep 2
  done
  [[ "$ready" == 1 ]]
  ...
  for route in orders payments; do
    code=$(curl -sS --max-time 10 -o /tmp/boomer-xintiandi-anon.json -w '%{http_code}' \
      -X POST -H 'Content-Type: application/json' -d '{}' "http://127.0.0.1:$port/api/public/storefront/$route")
    [[ "$code" == 401 ]]
    node -e 'JSON.parse(require("fs").readFileSync("/tmp/boomer-xintiandi-anon.json","utf8"))'
  done
}
...
case "${1:-}" in
  prepare|resume)
    ...
    cd "$release"
    npm run build:tencent > /tmp/boomer-xintiandi-build.log 2>&1
  ...
  activate)
    ...
    ;;
  *) echo 'Usage: deploy-xintiandi-listing-repair.sh prepare|activate' >&2; exit 2 ;;
esac
