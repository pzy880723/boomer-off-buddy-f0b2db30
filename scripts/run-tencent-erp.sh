...
if [[ -n "$LISTING_IMAGE_WORKER_OVERRIDE" ]]; then
  export HANDHELD_LISTING_IMAGE_WORKER_ENABLED="$LISTING_IMAGE_WORKER_OVERRIDE"
fi
if [[ "$BIND_PORT" != 3005 ]]; then
  export HANDHELD_RELEASE_WORKER_ENABLED=false HANDHELD_ITEM_SYNC_WORKER_ENABLED=false
  export HANDHELD_LISTING_IMAGE_WORKER_ENABLED=false
fi
set +a
