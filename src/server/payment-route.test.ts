...
describe('payment channel routing is snapshot driven', () => {
  ...
  it('分账历史单：全局切到普通商户后仍走原分账通道退款/回退', () => {
    ...
    assert.equal(resolvePaymentChannel({ id: 'p2', order_id: 'o2', payment_channel: 'legacy' }), 'legacy_split');
  ...
  it('未支付订单按下单时固化的 payment_route 路由', () => {
    ...
    assert.throws(() => resolveOrderChannel({ id: 'o7', payment_route: { mode: 'ordinary_wechat' } }), PaymentRouteError);
    assert.throws(() => resolveOrderChannel({ id: 'o8', payment_route: { mode: 'aggregator' } }), PaymentRouteError);
  });
});
...
describe('split settlement never degrades to headquarters collection', () => {
  ...
  it('全部就绪时通过', () => {
    assert.equal(assertSplitSettlementReady({ locationIds: ['a'], readyLocationIds: ['a', 'b'] }), undefined);
  });
});
...
describe('ordinary cross-store orders keep line attribution without profit share', () => {
  it('跨门店普通商户订单不发起微信分账，仅保留行级归属', () => {
    ...
    assert.deepEqual(plan, { profit_share: false, sub_orders: [], fulfillment_location_ids: ['loc-a', 'loc-b'] });
  });
});
