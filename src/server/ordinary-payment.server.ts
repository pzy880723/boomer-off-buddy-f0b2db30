...
let cached: ReturnType<typeof createWeChatPayClient> | null = null;
export function ordinaryPaymentRuntime() {
  ...
  if (!cached) cached = createWeChatPayClient({ ...settings, ...ordinaryPaymentSecrets(process.env),
    refundNotifyUrl: settings.notifyUrl,
  });
  return { ...settings, client: cached, store: ordinaryStore };
}
