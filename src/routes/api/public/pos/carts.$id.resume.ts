    ...
    handlers: {
      ...
      POST: async ({ request, params }) => {
        const { data: cart, error } = await supabaseAdmin
          ...
          .select(
            "id,location_id,customer_id,note,discount_snapshot,benefit_snapshot,status,pos_held_cart_items(id,sku_id,quantity,price_snapshot,ownership_snapshot,discount_eligible)",
          )
          .eq("id", params.id)
          .maybeSingle();
