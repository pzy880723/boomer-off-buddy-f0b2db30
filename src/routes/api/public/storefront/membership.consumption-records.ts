...
import { listMembershipConsumptionRecords } from "@/lib/membership/membership-repository.server";
import {
  STOREFRONT_CORS,
...
export const Route = createFileRoute("/api/public/storefront/membership/consumption-records")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        try {
          return storefrontJson({
            ok: true,
            data: await listMembershipConsumptionRecords(auth.customer.id),
          });
        } catch (error) {
