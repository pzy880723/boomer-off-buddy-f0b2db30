    ...
    handlers: {
      ...
      GET: async ({ request, params }) => {
        ...
        const { data: row, error } = await supabaseAdmin
          .from("japan_parcels")
          .select(
            "id, system_code, source_order_no, tracking_no, status, is_problem, receiver_name, receiver_address, total_weight_g, intl_pay_at, received_at, notes, created_at, status_timeline, intl_total_jpy, intl_exchange_rate, deleted_at",
          )
          .eq("id", parcelId)
          .maybeSingle();
        ...
        const { data: items, error: itemsErr } = await supabaseAdmin
          ...
          .select(
            "id, position, system_code, sub_order_no, item_title, item_title_cn, item_image_url, quantity, unit_price_jpy, item_total_jpy, weight_g, tariff_rate, tariff_category, pay_at, pay_method, notes, pack_pieces, pack_pieces_source, pack_unit_note",
          )
        ...
        const round2 = (v: number | null | undefined) =>
          v == null || !Number.isFinite(v) ? null : Math.round(Number(v) * 100) / 100;
