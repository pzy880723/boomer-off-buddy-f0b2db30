...
function SyncCenter() {
  ...
  const unbound = useQuery({ queryKey: ["yz-unbound"], queryFn: () => unboundFn() });
  ...
  return (
    <div className="p-6 space-y-4">
      <PageHeader
        title="有赞同步中心"
        description="本地 SKU 与有赞总部商品的绑定 / 库存推送状态总览"
        actions={
      ...
      <Tabs defaultValue="unbound">
        <TabsList>
          <TabsTrigger value="unbound">
            未绑定本地 SKU
            <span className="ml-1 text-[10px] text-muted-foreground">{unbound.data?.rows.length ?? 0}</span>
          </TabsTrigger>
          <TabsTrigger value="mismatch">
            <AlertTriangle className="mr-1 h-3 w-3" />
            库存不一致
            <span className="ml-1 text-[10px] text-muted-foreground">{mismatches.data?.rows.length ?? 0}</span>
          </TabsTrigger>
          <TabsTrigger value="error">
            推送失败
            <span className="ml-1 text-[10px] text-muted-foreground">{errors.data?.rows.length ?? 0}</span>
          </TabsTrigger>
        ...
        <TabsContent value="unbound" className="space-y-2">
          {(unbound.data?.rows ?? []).length === 0 ? (
            <EmptyState icon={Link2} title="所有 SKU 已绑定" />
          ) : (
            (unbound.data?.rows ?? []).map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded border p-2">
                ...
                <Button size="sm" onClick={() => setBindTarget({ id: s.id, name: s.name })}>
                  绑定有赞
                </Button>
              </div>
            ))
        ...
        <TabsContent value="mismatch" className="space-y-2">
          {(mismatches.data?.rows ?? []).length === 0 ? (
            <EmptyState icon={AlertTriangle} title="没有不一致的绑定" />
          ) : (
            (mismatches.data?.rows ?? []).map((l) => (
              <div key={l.id} className="flex items-center gap-3 rounded border p-2 text-sm">
        ...
        <TabsContent value="error" className="space-y-2">
          {(errors.data?.rows ?? []).length === 0 ? (
            ...
            (errors.data?.rows ?? []).map((l) => (
              <div key={l.id} className="space-y-1 rounded border border-destructive/40 p-2 text-sm">
        ...
        <TabsContent value="queue" className="space-y-2">
          {(queue.data?.rows ?? []).map((t) => (
            <div key={t.id} className="flex items-center gap-2 rounded border p-2 text-xs">
              <Badge
                variant={
                  t.status === "done" ? "secondary" : t.status === "failed" ? "destructive" : "outline"
                }
              >
                {t.status}
              </Badge>
              ...
              <span className="ml-auto text-muted-foreground">attempts {t.attempts}</span>
              {t.status === "failed" && (
                <Button
                  size="sm"
                  variant="outline"
