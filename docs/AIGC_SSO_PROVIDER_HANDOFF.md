...
- `roles` 来自 ERP 的 `public.user_roles` 表，取值为 `admin`、`moderator`、`user` 等；具体权限映射由 AIGC 侧定义。
- `shops` 来自 ERP 的 `public.user_location_perms`，只包含 `kind='shop'` 的库位（门店）。
...
辅助函数 `public.aigc_sso_cleanup_expired()`：删除 7 天以前的记录，可用 pg_cron 每天调度一次。
...
- **角色**（`public.user_roles`）
  - 多行：一个用户可以有多个角色
  - 字段 `role` 是枚举 `app_role`（`admin` / `moderator` / `user`）
  - 权限判定统一走 ERP 内的 `has_role(user_id, role)`；AIGC 侧只需保存拿到的 roles 数组
...
| `POST /api/public/sso/aigc-ticket` | ✅ 代码已上线到 Preview；等下一次发布后即可在 `https://boomer-off-buddy.lovable.app` 使用 |
| `POST /api/public/sso/aigc-exchange` | ✅ 代码已上线到 Preview；等下一次发布后即可在 `https://boomer-off-buddy.lovable.app` 使用 |
