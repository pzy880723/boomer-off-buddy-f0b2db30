-- BOOMER-OFF self-operated storefront and warehouse fulfillment core.
-- ERP remains the source of truth; storefront and handheld apps use server APIs only.
...
CREATE UNIQUE INDEX uniq_fulfillment_scan_client_op
  ON public.fulfillment_scans(device_id, client_op_id)
...
CREATE UNIQUE INDEX uniq_print_event_client_op
  ON public.print_events(device_id, client_op_id)
  WHERE client_op_id IS NOT NULL;

-- Sensitive commerce tables are server-only. Public and handheld routes use service_role.
...
BEGIN
  IF coalesce(array_length(p_listing_ids, 1), 0) = 0 THEN
    RAISE EXCEPTION 'order requires at least one listing';
  END IF;
  IF cardinality(p_listing_ids) <> (
    SELECT count(DISTINCT listing_id) FROM unnest(p_listing_ids) AS value(listing_id)
  ) THEN
  ...
  FOREACH v_listing_id IN ARRAY p_listing_ids LOOP
    SELECT * INTO v_listing FROM public.commerce_listings
    ...
    END IF;
    SELECT qty INTO v_stock FROM public.inv_stocks
      WHERE sku_id = v_listing.sku_id AND location_id = v_listing.location_id FOR UPDATE;
    IF coalesce(v_stock, 0) < 1 THEN
      RAISE EXCEPTION 'listing % is out of stock', v_listing_id;
    END IF;
    ...
    INSERT INTO public.inventory_reservations (
      order_id, listing_id, sku_id, location_id, expires_at
    ) VALUES (
      v_order.id, v_listing.id, v_listing.sku_id, v_listing.location_id, v_expires_at
    );
    UPDATE public.commerce_listings SET status = 'reserved', updated_at = now()
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM public.commerce_order_items WHERE order_id = p_order_id LOOP
    SELECT qty INTO v_stock FROM public.inv_stocks
      WHERE sku_id = v_item.sku_id AND location_id = v_item.location_id FOR UPDATE;
    IF coalesce(v_stock, 0) < 1 THEN
      RAISE EXCEPTION 'paid order inventory is no longer available for listing %', v_item.listing_id;
    END IF;
    PERFORM public.inv_apply_movement(
      v_item.sku_id, v_item.location_id, -1, 'commerce_sale', p_order_id, v_item.epc, p_provider_transaction_id
    );
    UPDATE public.commerce_listings SET status = 'sold', sold_at = p_paid_at, updated_at = now()
...
BEGIN
  ...
  SELECT * INTO v_tote FROM public.warehouse_totes
    WHERE code = p_tote_code AND location_id = p_location_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'tote not found at this location'; END IF;
  IF v_tote.status <> 'available' AND v_tote.current_fulfillment_id <> p_fulfillment_id THEN
    RAISE EXCEPTION 'tote is already in use';
  END IF;

  UPDATE public.warehouse_totes SET status = 'in_use', current_fulfillment_id = p_fulfillment_id, updated_at = now()
    WHERE id = v_tote.id;
  UPDATE public.fulfillments SET tote_id = v_tote.id, updated_at = now()
...
BEGIN
  IF p_client_op_id IS NOT NULL AND EXISTS (
    ...
    SELECT count(*), count(*) FILTER (WHERE picked_qty = expected_qty)
      INTO v_total, v_picked FROM public.fulfillment_items WHERE fulfillment_id = p_fulfillment_id;
  ...
  IF NOT FOUND THEN
    INSERT INTO public.fulfillment_scans(
      fulfillment_id, phase, code, code_type, result, rejection_reason,
      device_id, operator_id, client_op_id
    ) VALUES (
      p_fulfillment_id, 'pick', p_code, 'barcode', 'rejected', 'wrong_item',
  ...
  UPDATE public.fulfillment_items SET picked_qty = 1, picked_at = coalesce(picked_at, now())
    WHERE id = v_item.id;
  INSERT INTO public.fulfillment_scans(
    fulfillment_id, fulfillment_item_id, phase, code, code_type, result,
    device_id, operator_id, client_op_id
  ) VALUES (
  ...
  UPDATE public.fulfillments SET
    status = 'picking', picking_started_at = coalesce(picking_started_at, now()), updated_at = now()
    WHERE id = p_fulfillment_id;
  SELECT count(*), count(*) FILTER (WHERE picked_qty = expected_qty)
    INTO v_total, v_picked FROM public.fulfillment_items WHERE fulfillment_id = p_fulfillment_id;
