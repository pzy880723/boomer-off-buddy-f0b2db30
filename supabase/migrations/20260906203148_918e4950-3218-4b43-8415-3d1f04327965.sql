...
CREATE INDEX IF NOT EXISTS idx_fulfillments_location_status ON public.fulfillments (location_id, status);
...
AS $$
    ...
    SELECT
      ...
      CASE
        ...
        WHEN s.order_status = 'after_sale'
          OR s.payment_status IN ('refunding', 'refunded', 'partial_refunded')
          OR EXISTS (
  ...
  SELECT f.id, f.derived_status, f.f_total, f.f_done, count(*) OVER ()
  FROM filtered f
  ...
  WITH scoped AS (
    ...
    WHERE (p_location_ids IS NULL OR f.location_id = ANY (p_location_ids))
      ...
      AND (
        ...
        OR (p_status = 'pending_customer' AND EXISTS (
              SELECT 1 FROM public.fulfillment_shortages sh
              WHERE sh.fulfillment_id = f.id AND sh.status = 'pending_customer'
            ))
        OR (p_status NOT IN ('cancelled', 'pending_customer') AND f.status = p_status)
      )
  )
  SELECT s.id, s.order_cancelled, s.has_pending_customer, count(*) OVER ()
  FROM scoped s
  ORDER BY s.priority DESC NULLS LAST, s.created_at ASC
  LIMIT greatest(coalesce(p_limit, 20), 0)
  OFFSET greatest(coalesce(p_offset, 0), 0);
$$;
