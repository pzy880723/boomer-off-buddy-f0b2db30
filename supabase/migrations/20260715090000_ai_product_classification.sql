...
-- ERP taxonomy remains the source of truth; models may only select active leaf codes.
...
-- Preserve legacy references while removing old roots from new-product selection.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'inv_skus_category_source_check'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'inv_skus_category_confidence_check'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'inv_skus_classification_status_check'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'inv_skus_ai_suggested_price_check'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'sku_classifications_read_authenticated'
  ) THEN
    CREATE POLICY sku_classifications_read_authenticated
      ON public.inv_sku_classifications
...
GRANT SELECT ON public.inv_sku_classifications TO authenticated;
GRANT ALL ON public.inv_sku_classifications TO service_role;

COMMENT ON COLUMN public.inv_skus.attributes IS
  'AI extracted structured product attributes: origin, era, material, craft, object type, colors, function and missing parts.';
COMMENT ON TABLE public.inv_sku_classifications IS
  'Immutable-style AI recognition audit records. Corrections append metadata and link to the final SKU.';
