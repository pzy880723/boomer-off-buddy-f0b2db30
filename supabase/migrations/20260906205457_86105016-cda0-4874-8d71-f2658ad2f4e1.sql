...
AS $$
  ...
  SELECT page.id, page.derived_status, page.f_total, page.f_done, c.total
  FROM counted c
  LEFT JOIN LATERAL (
    SELECT f.id, f.derived_status, f.f_total, f.f_done
    FROM filtered f
...
AS $$
  ...
  SELECT page.id, page.order_cancelled, page.has_pending_customer, c.total
  FROM counted c
  LEFT JOIN LATERAL (
    SELECT s.id, s.order_cancelled, s.has_pending_customer
    FROM scoped s
