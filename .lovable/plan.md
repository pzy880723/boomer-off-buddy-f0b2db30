...
## 6. 生产最小补丁部署状态记录（2026-09-13，仅记录，非本环境执行）

- 用户已明确允许：过期测试订单按原规则取消/释放库存。
- 用户确认 Codex 已将 commit `0c2dfd2733d2ef7a27cc9cd1387a1e86029a2956` 的最小普通支付错误码兼容修复单独应用到腾讯生产基线 `1e7a4ee`。
- 生产运行目录：`/var/www/boomer-erp/releases/1e7a4ee-payment-compat-20260913`，PID `1861359`。
- 已核对实际运行路径/工作目录；支付及 Supabase 配置指纹未变；旧版本及进程配置已保留。
- 候选实际运行代码的 16 个无网络回归测试通过；公网 `taxonomy/products` GET 返回 200。
- 2026-09-13 09:32:18 CST 定时对账由 `failed=1` 恢复为 `attempted=1/succeeded=1/failed=0`。
- 未发起真实预支付或付款。
- 小程序最小修复开发版 `0.2.4` 已上传；微信公众平台浏览器操作被工具安全策略禁止，需用户自行设为体验版。
- **不视为手机付款闭环验收，也不视为全量腾讯迁移完成**。
